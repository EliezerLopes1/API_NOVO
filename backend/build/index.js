...
app.put("/editar-info", function (req, res) {
    ...
    var SQL = ("UPDATE Usuarios SET usuario_nome = '" + nome + "', nome_usuario = '" + username + "' WHERE usuario_ID = '5f86aeec-ee1e-4405-8c48-7129cdd72b41'");
    DB.query(SQL, function (err, result) {
        if (err) {
            console.log(err);
        }
...
app.get("/read", function (_, res) {
    var SQL = "SELECT usuario_nome, usuario_email, nome_usuario, usuario_tipo, to_char(usuario_data_criacao, 'DD/MM/YYYY') FROM usuarios WHERE usuario_ID = '5f86aeec-ee1e-4405-8c48-7129cdd72b41'";
...
app.get("/readAcoes", function (_, res) {
    var SQL = "SELECT tipo_acao, to_char(data_acao, 'DD/MM/YYYY HH24:MI:SS')FROM acoes WHERE id_usuario = '5f86aeec-ee1e-4405-8c48-7129cdd72b41'";
    DB.query(SQL, function (err, data) {
        if (err)
            return res.json(err);
        return res.status(200).json(data.rows);
